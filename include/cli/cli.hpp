#pragma once

#include <charconv>
#include <concepts>
#include <deque>
#include <expected>
#include <format>
#include <functional>
#include <optional>
#include <print>
#include <span>
#include <string>
#include <string_view>
#include <unordered_map>
#include <variant>
#include <vector>

namespace cli {

// ---- Concepts ---------------------------------------------------------------

template<typename T>
concept Parseable =
    std::same_as<T, bool>        ||
    std::same_as<T, int>         ||
    std::same_as<T, long>        ||
    std::same_as<T, double>      ||
    std::same_as<T, float>       ||
    std::same_as<T, std::string>;

// ---- Error types ------------------------------------------------------------

enum class ParseErrorKind {
    UnknownOption,
    MissingValue,
    InvalidValue,
    MissingRequired,
};

struct ParseError {
    ParseErrorKind kind;
    std::string    message;

    static ParseError unknown_option(std::string_view opt) {
        return {ParseErrorKind::UnknownOption,
                std::format("unknown option: '{}'", opt)};
    }
    static ParseError missing_value(std::string_view opt) {
        return {ParseErrorKind::MissingValue,
                std::format("option '{}' requires a value", opt)};
    }
    static ParseError invalid_value(std::string_view opt, std::string_view val) {
        return {ParseErrorKind::InvalidValue,
                std::format("invalid value '{}' for option '{}'", val, opt)};
    }
    static ParseError missing_required(std::string_view opt) {
        return {ParseErrorKind::MissingRequired,
                std::format("required option '{}' not provided", opt)};
    }
};

// ---- Value storage ----------------------------------------------------------

using Value = std::variant<bool, int, long, double, float, std::string>;

// ---- Internal option descriptor ---------------------------------------------

struct OptionDesc {
    std::string              name;
    std::vector<std::string> aliases;
    std::string              help;
    std::string              type_name;
    bool                     is_flag  = false;
    bool                     required = false;
    std::optional<Value>     default_value;

    std::function<std::expected<Value, ParseError>(std::string_view key,
                                                   std::string_view raw)> parser;
};

// ---- ParseResult ------------------------------------------------------------

class ParseResult {
public:
    template<Parseable T>
    [[nodiscard]] T get(std::string_view name) const {
        auto it = values_.find(std::string(name));
        if (it == values_.end())
            throw std::out_of_range(
                std::format("option '{}' not found in parse result", name));
        return std::get<T>(it->second);
    }

    template<Parseable T>
    [[nodiscard]] std::optional<T> get_optional(std::string_view name) const {
        auto it = values_.find(std::string(name));
        if (it == values_.end()) return std::nullopt;
        return std::get<T>(it->second);
    }

    [[nodiscard]] bool has(std::string_view name) const {
        return values_.contains(std::string(name));
    }

    [[nodiscard]] std::span<const std::string> positional() const {
        return positional_;
    }

private:
    std::unordered_map<std::string, Value> values_;
    std::vector<std::string>               positional_;

    friend class Parser;
};

// ---- Option builder ---------------------------------------------------------

// Uses an index into the parser's options deque so it is safe even if the
// deque grows during construction of the parser.

class Parser; // forward

template<Parseable T>
class OptionBuilder {
public:
    OptionBuilder(Parser& parser, std::size_t idx)
        : parser_(parser), idx_(idx) {}

    OptionBuilder& alias(std::string a);
    OptionBuilder& help(std::string h);
    OptionBuilder& required();

    // Default values are not allowed for flags (bool without explicit value).
    OptionBuilder& default_value(T val) requires (!std::same_as<T, bool>) {
        desc().default_value = Value{val};
        return *this;
    }

private:
    Parser&     parser_;
    std::size_t idx_;

    OptionDesc& desc();
};

// ---- Parser -----------------------------------------------------------------

class Parser {
public:
    explicit Parser(std::string name, std::string description = "")
        : name_(std::move(name)), description_(std::move(description)) {}

    // Add a typed option that consumes the next token as its value.
    template<Parseable T>
    OptionBuilder<T> option(std::string name) {
        auto& desc    = options_.emplace_back();
        desc.name     = name;
        desc.is_flag  = false;
        desc.type_name = type_name_for<T>();
        desc.parser   = make_value_parser<T>();
        return {*this, options_.size() - 1};
    }

    // Add a boolean flag that takes no value (present → true).
    OptionBuilder<bool> flag(std::string name) {
        auto& desc       = options_.emplace_back();
        desc.name        = name;
        desc.is_flag     = true;
        desc.type_name   = "flag";
        desc.default_value = Value{false};
        desc.parser = [](std::string_view, std::string_view)
                          -> std::expected<Value, ParseError> {
            return Value{true};
        };
        return {*this, options_.size() - 1};
    }

    // Parse argc/argv. Returns ParseResult or a descriptive ParseError.
    [[nodiscard]] std::expected<ParseResult, ParseError>
    parse(int argc, char* argv[]) const {
        ParseResult result;

        // Seed with defaults.
        for (auto& opt : options_) {
            if (opt.default_value)
                result.values_[opt.name] = *opt.default_value;
        }

        std::vector<std::string_view> args;
        args.reserve(static_cast<std::size_t>(argc - 1));
        for (int i = 1; i < argc; ++i)
            args.emplace_back(argv[i]);

        bool past_separator = false;

        for (std::size_t i = 0; i < args.size(); ++i) {
            auto arg = args[i];

            // Positional: anything after "--", bare "-", or non-flag token.
            if (past_separator || arg == "-" || !arg.starts_with('-')) {
                result.positional_.emplace_back(arg);
                continue;
            }

            if (arg == "--") {
                past_separator = true;
                continue;
            }

            // Split --key=value if present.
            std::string_view           key = arg;
            std::optional<std::string_view> inline_val;

            if (auto eq = arg.find('='); eq != std::string_view::npos) {
                key        = arg.substr(0, eq);
                inline_val = arg.substr(eq + 1);
            }

            const OptionDesc* opt = find_option(key);
            if (!opt)
                return std::unexpected(ParseError::unknown_option(key));

            if (opt->is_flag) {
                result.values_[opt->name] = Value{true};
                continue;
            }

            // Consume value.
            std::string_view val;
            if (inline_val) {
                val = *inline_val;
            } else if (i + 1 < args.size()) {
                val = args[++i];
            } else {
                return std::unexpected(ParseError::missing_value(key));
            }

            auto parsed = opt->parser(key, val);
            if (!parsed) return std::unexpected(parsed.error());
            result.values_[opt->name] = *parsed;
        }

        // Validate required options.
        for (auto& opt : options_) {
            if (opt.required && !result.values_.contains(opt.name))
                return std::unexpected(ParseError::missing_required(opt.name));
        }

        return result;
    }

    // Convenience: parse and exit with an error message on failure.
    [[nodiscard]] ParseResult parse_or_exit(int argc, char* argv[]) const {
        auto result = parse(argc, argv);
        if (!result) {
            std::println(stderr, "error: {}", result.error().message);
            std::println(stderr, "Try '{} --help' for usage.", name_);
            std::exit(1);
        }
        return std::move(*result);
    }

    void print_help() const {
        std::println("Usage: {} [options]", name_);
        if (!description_.empty())
            std::println("\n{}", description_);
        std::println("\nOptions:");

        for (auto& opt : options_) {
            // Build "  --name, -n <type>"
            std::string lhs = "  " + opt.name;
            for (auto& a : opt.aliases)
                lhs += ", " + a;
            if (!opt.is_flag)
                lhs += std::format(" <{}>", opt.type_name);

            // Align help text at column 30.
            constexpr std::size_t col = 30;
            if (lhs.size() < col)
                lhs.append(col - lhs.size(), ' ');
            else
                lhs += "\n" + std::string(col, ' ');

            std::string rhs = opt.help;
            if (opt.default_value && !opt.is_flag)
                rhs += std::format(" [default: {}]",
                                   value_to_string(*opt.default_value));
            if (opt.required)
                rhs += " (required)";

            std::println("{}{}", lhs, rhs);
        }
    }

private:
    std::string             name_;
    std::string             description_;
    std::deque<OptionDesc>  options_; // deque gives stable references

    template<Parseable T> friend class OptionBuilder;

    const OptionDesc* find_option(std::string_view key) const {
        for (auto& opt : options_) {
            if (opt.name == key) return &opt;
            for (auto& a : opt.aliases)
                if (a == key) return &opt;
        }
        return nullptr;
    }

    template<Parseable T>
    static std::string type_name_for() {
        if constexpr (std::same_as<T, bool>)        return "bool";
        else if constexpr (std::same_as<T, int>)    return "int";
        else if constexpr (std::same_as<T, long>)   return "long";
        else if constexpr (std::same_as<T, double>) return "double";
        else if constexpr (std::same_as<T, float>)  return "float";
        else                                         return "string";
    }

    template<Parseable T>
    static auto make_value_parser() {
        return [](std::string_view key, std::string_view val)
                   -> std::expected<Value, ParseError> {
            if constexpr (std::same_as<T, std::string>) {
                return Value{std::string(val)};
            } else if constexpr (std::same_as<T, bool>) {
                if (val == "true"  || val == "1" || val == "yes") return Value{true};
                if (val == "false" || val == "0" || val == "no")  return Value{false};
                return std::unexpected(ParseError::invalid_value(key, val));
            } else if constexpr (std::same_as<T, double> || std::same_as<T, float>) {
                // std::from_chars for fp requires macOS 26+ SDK; use stod/stof.
                try {
                    std::size_t pos{};
                    double d = std::stod(std::string(val), &pos);
                    if (pos != val.size())
                        return std::unexpected(ParseError::invalid_value(key, val));
                    return Value{static_cast<T>(d)};
                } catch (...) {
                    return std::unexpected(ParseError::invalid_value(key, val));
                }
            } else {
                T out{};
                auto [ptr, ec] = std::from_chars(
                    val.data(), val.data() + val.size(), out);
                if (ec != std::errc{} || ptr != val.data() + val.size())
                    return std::unexpected(ParseError::invalid_value(key, val));
                return Value{out};
            }
        };
    }

    static std::string value_to_string(const Value& v) {
        return std::visit([]<typename T>(const T& val) -> std::string {
            if constexpr (std::same_as<T, std::string>) return val;
            else if constexpr (std::same_as<T, bool>)   return val ? "true" : "false";
            else return std::to_string(val);
        }, v);
    }
};

// ---- OptionBuilder method bodies (need Parser definition) -------------------

template<Parseable T>
OptionDesc& OptionBuilder<T>::desc() {
    return parser_.options_[idx_];
}

template<Parseable T>
OptionBuilder<T>& OptionBuilder<T>::alias(std::string a) {
    desc().aliases.push_back(std::move(a));
    return *this;
}

template<Parseable T>
OptionBuilder<T>& OptionBuilder<T>::help(std::string h) {
    desc().help = std::move(h);
    return *this;
}

template<Parseable T>
OptionBuilder<T>& OptionBuilder<T>::required() {
    desc().required = true;
    return *this;
}

} // namespace cli
